...
FVector UHeroMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
void UHeroMovementComponent::SetGravityDirection(FVector NewGravityDirection)
{
	CustomGravityDirection = NewGravityDirection.GetSafeNormal();
}

void UHeroMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
float UHeroMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void UHeroMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice /*= 0.f*/, const FVector& MoveDelta /*= FVector::ZeroVector*/)
{
	if (CharacterOwner)
	{
...
void UHeroMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UHeroMovementComponent::MaintainHorizontalGroundVelocity()
{
	return;
...
float UHeroMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UHeroMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
...
void UHeroMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
FVector UHeroMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const	// OK
{
...
void UHeroMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult /*= NULL*/)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void UHeroMovementComponent::SimulateMovement(float DeltaTime)
{
...
FVector UHeroMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector UHeroMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * GetClampedToMaxSizePrecise(InputAcceleration, 1.0f);
}

FVector UHeroMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
...
FVector UHeroMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	const FVector GravityDirection = GetGravityDirection();
...
FVector UHeroMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void UHeroMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
bool UHeroMovementComponent::DoJump(bool bReplayingMoves)
{
...
FVector UHeroMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UHeroMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
void UHeroMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
...
void UHeroMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
bool UHeroMovementComponent::FloorSweepTest(struct FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UHeroMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UHeroMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetCapsuleAxisZ();
...
bool UHeroMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius /*= true*/) const
{
...
bool UHeroMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UHeroMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UHeroMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& Hit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult /*= NULL*/)
{
...
void UHeroMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode /*= 0*/)
{
...
void UHeroMovementComponent::SetDefaultMovementMode()
{
...
void UHeroMovementComponent::AdjustFloorHeight()
{
...
bool UHeroMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UHeroMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UHeroMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations)
{
...
void UHeroMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UHeroMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
bool UHeroMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void UHeroMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	OutFloorResult.Clear();
...
bool UHeroMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const FVector CapsuleDown = GetCapsuleAxisZ() * -1.0f;
...
FVector UHeroMovementComponent::GetGravity() const
{
	if (!CustomGravityDirection.IsZero())
	{
...
FVector UHeroMovementComponent::GetComponentDesiredAxisZ() const
{
	return GetGravityDirection(true) * -1.0f;
}

void UHeroMovementComponent::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
		return;
...
inline FQuat UHeroMovementComponent::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

inline FVector UHeroMovementComponent::GetCapsuleAxisX() const
{
...
inline FVector UHeroMovementComponent::GetCapsuleAxisZ() const
{
...
// Version that does not use inverse sqrt estimate, for higher precision.
inline FVector UHeroMovementComponent::GetSafeNormalPrecise(const FVector& V)
{
	const float VSq = V.SizeSquared();
	if (VSq < SMALL_NUMBER)
	{
...
bool UHeroMovementComponent::IsWithinEdgeToleranceNew(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (CapsuleLocation + CapsuleDown * ((TestImpactPoint - CapsuleLocation) | CapsuleDown) - TestImpactPoint).SizeSquared();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(KINDA_SMALL_NUMBER, CapsuleRadius - SWEEP_EDGE_REJECT_DISTANCE));
