...
//UGravMovementComponent::UGravMovementComponent()
//{
...
UGravMovementComponent::UGravMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UGravMovementComponent::InitializeComponent()
{
	Super::InitializeComponent();
}


FVector UGravMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
void UGravMovementComponent::SetGravityDirection(FVector NewGravityDirection)
{
	CustomGravityDirection = NewGravityDirection.GetSafeNormal();
}

void UGravMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
float UGravMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void UGravMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void UGravMovementComponent::PerformMovement(float DeltaTime)
{
	if (!HasValidData())
	{
		return;
...
void UGravMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice /*= 0.f*/, const FVector& MoveDelta /*= FVector::ZeroVector*/)
{
	if (CharacterOwner)
	{
...
void UGravMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UGravMovementComponent::MaintainHorizontalGroundVelocity()
{
	return;
...
float UGravMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UGravMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
...
void UGravMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
FVector UGravMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const	// OK
{
...
void UGravMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult /*= NULL*/)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void UGravMovementComponent::SimulateMovement(float DeltaTime)
{
...
FVector UGravMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector UGravMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * GetClampedToMaxSizePrecise(InputAcceleration, 1.0f);
}

FVector UGravMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
...
void UGravMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
	const float DesiredDist = Delta.Size();
...
void UGravMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
FVector UGravMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	const FVector GravityDirection = GetGravityDirection();
...
FVector UGravMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void UGravMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
bool UGravMovementComponent::DoJump(bool bReplayingMoves)
{
...
FVector UGravMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UGravMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
void UGravMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	...
	{
		UGravMovementComponent* MutableThis = const_cast<UGravMovementComponent*>(this);
...
void UGravMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
bool UGravMovementComponent::FloorSweepTest(struct FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UGravMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UGravMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetCapsuleAxisZ();
...
bool UGravMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius /*= true*/) const
{
...
bool UGravMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UGravMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UGravMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& Hit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult /*= NULL*/)
{
...
void UGravMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode /*= 0*/)
{
...
void UGravMovementComponent::SetDefaultMovementMode()
{
...
void UGravMovementComponent::AdjustFloorHeight()
{
...
bool UGravMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UGravMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UGravMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations)
{
...
void UGravMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UGravMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
bool UGravMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void UGravMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	OutFloorResult.Clear();
...
bool UGravMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const FVector CapsuleDown = GetCapsuleAxisZ() * -1.0f;
...
FVector UGravMovementComponent::GetGravity() const
{
	if (!CustomGravityDirection.IsZero())
	{
...
FVector UGravMovementComponent::GetComponentDesiredAxisZ() const
{
	return GetGravityDirection(true) * -1.0f;
}

void UGravMovementComponent::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
		return;
...
inline FQuat UGravMovementComponent::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

inline FVector UGravMovementComponent::GetCapsuleAxisX() const
{
...
inline FVector UGravMovementComponent::GetCapsuleAxisZ() const
{
...
// Version that does not use inverse sqrt estimate, for higher precision.
inline FVector UGravMovementComponent::GetSafeNormalPrecise(const FVector& V)
{
	const float VSq = V.SizeSquared();
	if (VSq < SMALL_NUMBER)
	{
...
bool UGravMovementComponent::IsWithinEdgeToleranceNew(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (CapsuleLocation + CapsuleDown * ((TestImpactPoint - CapsuleLocation) | CapsuleDown) - TestImpactPoint).SizeSquared();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(KINDA_SMALL_NUMBER, CapsuleRadius - SWEEP_EDGE_REJECT_DISTANCE));
